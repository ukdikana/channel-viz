(function ( $ ){
	...
	*/
	...
	var defaultKey		= 'YlczYuKio7zObc2F0MdEJSeXt5rqb74r5vqFNnzkNVwrbpa0', // Unique master Xively API key to be used as a default
		defaultFeeds	= [789742651], // Comma separated array of Xively Feed ID numbers
			...
			xively.feed.history(id, {  duration: "6hours", interval: 30 }, function (data) {
				if(data.id == id) {
					...
					$('#feed-' + data.id + ' .duration-90').click(function() {
						$('#loadingData').foundation('reveal', 'open');
						updateFeeds(data.id, thisFeedDatastreams, '1days', 10800);
						return false;
					});
